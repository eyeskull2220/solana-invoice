...
One offline flow: **form → Dutch offerte → voorbeeld → herinnering**. Line items travel with the job. Demo-branded for a fake freelancer (**Studio Noord**). Rebrand once, reuse for many clients.

Printed third document is **VOORBEELD**, not FACTUUR. Offerte stays Offerte. Demo KBO/BTW is always **nog niet toegekend** — never a placeholder BE number.
...
3. **Voorbeeld** — same lines as the offerte. Prints **VOORBEELD**, not FACTUUR. Download standalone HTML.
4. **Herinnering** — overdue letter from the voorbeeld totals. Download standalone HTML, or open mailto.
...
- KBO/BTW: **nog niet toegekend** (no placeholder BE number)

No real client names. No live personal surnames. No Gmail. No live company inboxes. Generic `*.example` only.
...
3. Leave **KBO/BTW: nog niet toegekend** until they have a real number. Do not invent a placeholder BE number.
...
6. Set a real offerte / document number series, dates, line items, and due window.
7. Leave the sell-page pay-to as the treasury address unless you are selling the kit itself.
8. Do **not** delete the Peppol answers. Do **not** retitle VOORBEELD as FACTUUR until they issue a real invoice.
...
- This kit does **not** make B2B invoices Peppol-compliant. We do not run a Peppol Access Point. We do not send live Peppol documents. The third page is a **VOORBEELD**.
...
- Fake or placeholder BE VAT/KBO numbers in the demo
