...
const PAY_URL =
  ...
  "&memo=" + INVOICE +
  "&label=" + encodeURIComponent("Dual-chain USDC invoice");
...
ok(html.includes('data-invoice-number="' + INVOICE + '"'), "invoice number data attr");
ok(html.includes(">" + INVOICE + "<"), "invoice number in DOM");
ok((html.match(/INV-20260826/g) || []).length >= 1, "invoice number present");
ok(!/INV-\d{8}-\d/.test(html), "no extra invoice-number suffixes");
...
ok(email.length === 0, "emails: " + email.join(","));
